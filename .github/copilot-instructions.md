...
- Root solution: `src/eShopLite-Aspire.slnx`
- AppHost (Aspire orchestrator): `src/eShopAppHost` (entry point: `src/eShopAppHost/Program.cs`)
- Store UI: `src/Store`
...
1. Create two Web API projects under `src/` if they do not exist: `SingleAgentDemo` and `MultiAgentDemo` (target `net9.0`). Add both projects to `src/eShopLite-Aspire.slnx`.
...
- Update `src/eShopAppHost/Program.cs` to register the services with Aspire so they are discoverable by other services in the solution and receive centralized logging/health.
...
7. Register the new services with the Aspire AppHost in `src/eShopAppHost/Program.cs` so they are discoverable and receive logging/health by Aspire (see "Must-preserve instruction" above).
...
- `src/eShopAppHost/Program.cs` — register services with Aspire (service discovery, logging, health checks).
- `src/eShopLite-Aspire.slnx` — add new projects here.
...
- Run `dotnet build src/eShopLite-Aspire.slnx -c Debug` and ensure solution builds.
